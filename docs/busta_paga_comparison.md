...
| --- | --- | --- |
| pytesseract + markitdown | 708.23 | 44.19 |
| MarkItDownNet | 977.59 | – |
...
- Con l'opzione `ParagraphGapThreshold` la versione .NET inserisce righe vuote quando individua un distacco verticale, rendendo il Markdown più leggibile.
- `markitdown` applica una normalizzazione più aggressiva e in generale produce un layout leggermente più strutturato; `MarkItDownNet` è più compatto ma integra l'OCR in un'unica passata.
