# Busta paga Markdown comparison

Conversion of `dataset/busta_paga_internet.jpeg` with Microsoft's
`markitdown` CLI and the .NET `MarkItDownNet` library.
...
The CLI produced an empty file despite Tesseract data being available.
...
MarkItDownNet successfully extracted the payroll text via Tesseract and
returned it as plain Markdown, whereas `markitdown` yielded no content.
