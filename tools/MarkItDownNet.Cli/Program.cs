...
using MarkItDownNet;
using MarkItDownNet.Cli;
using SkiaSharp;
using System.Runtime.InteropServices;

if (args.Length == 0)
{
    Console.WriteLine("Usage: markitdownnet <convert|bench|ocr|mdgen|mdcompare> [options]");
    return;
}
...
switch (args[0])
{
    ...
    case "ocr":
        ...
        break;
    case "mdgen":
        MdGenCommand(args.Skip(1).ToArray());
        break;
    case "mdcompare":
        MdCompareCommand(args.Skip(1).ToArray());
        break;
    default:
...
static void MdGenCommand(string[] args)
{
    string txtDir = GetOption(args, "--txt-dir") ?? throw new ArgumentException("--txt-dir required");
    string outDir = GetOption(args, "--out-dir") ?? throw new ArgumentException("--out-dir required");
    string enginesOpt = GetOption(args, "--engines") ?? "markitdownnet";
    string pythonExe = GetOption(args, "--python-exe") ?? "python3";
    var engines = enginesOpt.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
    foreach (var file in Directory.GetFiles(txtDir, "*.mdready.txt", SearchOption.AllDirectories))
    {
        var rel = Path.GetRelativePath(txtDir, file);
        var outRel = rel.Substring(0, rel.Length - ".mdready.txt".Length) + ".md";
        foreach (var e in engines)
        {
            var outPath = Path.Combine(outDir, e, outRel);
            Directory.CreateDirectory(Path.GetDirectoryName(outPath)!);
            var sw = Stopwatch.StartNew();
            if (e == "markitdown")
            {
                var psi = new ProcessStartInfo(pythonExe, $"tools/md_from_text_markitdown.py \"{file}\"") { RedirectStandardOutput = true, UseShellExecute = false };
                using var p = Process.Start(psi)!;
                var output = p.StandardOutput.ReadToEnd();
                p.WaitForExit();
                File.WriteAllText(outPath, output);
                sw.Stop();
                Console.WriteLine($"engine=md file={rel} time_ms={sw.ElapsedMilliseconds} out={outPath}");
            }
            else if (e == "markitdownnet")
            {
                var txt = File.ReadAllText(file);
                var md = MarkdownFromText.Generate(txt);
                File.WriteAllText(outPath, md);
                sw.Stop();
                Console.WriteLine($"engine=mdnet file={rel} time_ms={sw.ElapsedMilliseconds} out={outPath}");
            }
        }
    }
}

static void MdCompareCommand(string[] args)
{
    ...
    var runConfig = new { os = RuntimeInformation.OSDescription, dotnet = Environment.Version.ToString(), python = GetPythonVersion("python"), timings_unit = "ms" };
    ...
    foreach (var refFile in Directory.GetFiles(baseDir, "*.md", SearchOption.AllDirectories))
    {
        ...
        sw.Stop();
        files.Add(new MdFileResult { dataset = dataset, paths = new FilePaths { ref_md = refFile, hyp_md = hypFile, src_txt = txtPath }, metrics = new Metrics { text = textM, lines = lineM, structure = new StructurePair { ref_struct = structRef, hyp_struct = structHyp } }, timing_ms = sw.ElapsedMilliseconds });
    }
    ...
    Directory.CreateDirectory(Path.GetDirectoryName(summaryMd)!);
    bool attention = global.TokenF1 < 0.97 || global.LineF1 < 0.94;
    var violations = new List<string>();
    bool fail = global.TokenF1 < 0.95 || global.LineF1 < 0.92 || DatasetViolations(byDataset, violations);
    File.WriteAllText(summaryMd, BuildSummary(runConfig, byDataset, global, files, attention, fail, violations));
    if (fail)
    {
...
static string BuildSummary(object runConfig, Dictionary<string, DatasetAgg> byDataset, DatasetAgg global, List<MdFileResult> files, bool attention, bool fail, List<string> violations)
{
